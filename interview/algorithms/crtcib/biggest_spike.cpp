/*
given an array of integers. find the length of the largest spike you can create. A spike is formed by creating a strictly increasing sequence followed by a scritcyly decreasing sequence. Each element can only be used once. You need not select all elements in the array.
...
int main() {
    ...
    std::set<int, std::less<int>> increasing_seq;
    std::set<int, std::greater<int>> decreasing_seq;
    ...
    int sz = increasing_seq.size() + decreasing_seq.size();
