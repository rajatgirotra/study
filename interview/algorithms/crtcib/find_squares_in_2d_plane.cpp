...
struct Square {
    ...
    Square(Point a, Point b, Point c, Point d) : m_p1(a), m_p2(b), m_p3(c), m_p4(d) {
    ...
    auto operator <=> (const Square& rhs) const noexcept = default;
...
std::uint64_t euclidean(const Point& a, const Point& b) {
    ...
    return pow(a.m_x - b.m_x, 2) + pow(a.m_y - b.m_y, 2);
}
...
auto get_squares(const std::array<int, N>& x, const std::array<int, N>& y) {
    ...
    for(int i = 0; i < N - 1; ++i) {
        ...
        for(int j = i+1; j < N; ++j) {
            ...
            auto& dSet = dMap[d];
            dSet.emplace(std::make_pair(a, b));
        }
    }
