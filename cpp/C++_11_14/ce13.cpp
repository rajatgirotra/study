...
If someone forgets anyway, then the compiler will evaulate it at run time, this means that the throw will be evaluated and put in the executable. To avoid a runtime error and make a link time error, there is a cool technique. Just add an unresolved symbol in throw and we'll get a link time error.
