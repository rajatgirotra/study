...
By default, the return type of the thread function is ignored. Use std::promise if you are interested in the the thread funtion return value. If a thread throws an exception, std::terminate is called.

thread has a nested class id. (std::thread::id) This class is used to get an id of a thread. The important use of this class is to use it in associative containers as key. Other important functions are.
...
thread ctor might throw exception std::system_error if the thread could not be started. The exception may represent the error condition std::errc::resource_unavailable_try_again or another implementation-specific error condition
