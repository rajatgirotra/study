...
struct plus_f
{
    typedef typename mpl::plus<Seq1, Seq2>::type type;
};

template <typename T, typename D1, typename D2>
quantity<T, typename mpl::transform<D1, D2, plus_f<_1, _2> >::type >
operator * (const quantity<T, D1>& lhs, const quantity<T, D2>& rhs)
{
    typedef typename mpl::transform<D1, D2, plus_f<_1, _2> >::type newDim;
    return quantity<T, newDim> (lhs.value() * rhs.value());
}

/*
 * Both of the above solutions are correct, because the argument to transform is a Lambda expression.
 * ie.
 * A Metafunction class in first case
 * A Placeholder expression in second case.
 */
