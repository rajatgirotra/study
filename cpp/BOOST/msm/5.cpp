...
/* Things to note"
 * A new event error_found.
 * two new states: "AllOk" and "ErrorMode"
 * ErrorMode state derives from terminate_state<>
 * transition table has been enhanced for the new state.
 * the initial_state typedef has been modified to include a new region.
 * the pstate() function now makes use of nr_regions::value which tells you the number of regions in your state machine. Default is 1.
 * At any time, there are two states (one from each region) active.
 * Lines 264 and 265 do not trigger any transitions because we reach a terminal state before.
 */
