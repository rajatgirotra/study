...
----------------------------------------
There is no central server which stores all the bitcoin transactions that have happened till now. Infact thousands of servers exist in the world which keep track of all the bitcoins transactions and how much bitcoins are owned by whom. These servers are called "a full node". Anyone in the world can setup and run a full node. Each node has the full public ledger - i.e a record of every bitcoin transaction that ever happened, in history, all the way to back to the very beginning of bitcoin. As of Aug 2023, the size of the bitcoin ledger is more than 500 GB.
