 ...
 *
 * The variable n in the foobar() function is declared as static. This means that n is instantiated only once and retains its value between function calls.
 * Returning a reference to a static local variable avoids creating a temporary object that could be moved. Static variables are not eligible for RVO because
 * they exist in a fixed location and their lifetime extends beyond the function scope. As a result, the returned object is not a temporary rvalue but an
 * lvalue reference to a static object. So the copy constructor would be called
 */
